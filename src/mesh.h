...
//
#ifndef HD_RDN_MESH_H
#define HD_RDN_MESH_H
...
///
class HdRdnMesh final : public HdMesh 
{
public:
    HF_MALLOC_TAG_NEW("new HdRdnMesh");

    /// HdRdnMesh constructor.
    ///   \param id The scene-graph path to this mesh.
    HdRdnMesh(SdfPath const& id);

    /// HdRdnMesh destructor.
    ~HdRdnMesh() override = default;
...
protected:
    ...
    HdRdnMesh(const HdRdnMesh&) = delete;
    HdRdnMesh &operator =(const HdRdnMesh&) = delete;
};
